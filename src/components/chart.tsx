...
import type { ChartPoint, JoinedPoint } from "../types/chart";
...
export default function Chart() {
	...
	useEffect(() => {
		if (dataFng && dataFng.length > 0 && dataPrice && dataPrice.length > 0) {
			...
			const joinedData: JoinedPoint[] = Array.from(keys)
				.filter((key) => map1.has(key) && map2.has(key))
				.map((key) => ({
					timestamp: key,
	...
	useEffect(() => {
		if (joinedData && joinedData.length > 0) {
			...
			joinedData.forEach((point, idx) => {
				...
				const movingAverage = +(
					joinedData.slice(idx - sma, idx).reduce((acc, cur) => {
						return acc + parseInt(cur.fng, 10);
					}, 0) / sma
				).toFixed(2);
			...
			if (chartSettings.scaleToFit) {
				const startPoint = chartArr[sma];
				if (startPoint) {
					const start = startPoint.ma_value;
					for (let i = 0; i < sma; i++) {
						const point = chartArr[i];
						if (point) {
							point.ma_value = start;
						}
					}
				}
			}
